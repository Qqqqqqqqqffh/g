...
def generate_image(leaderboard):
    ...
    draw.text((width // 2, 70), f"Обновлено: {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')} UTC",
              font=font_text, fill="gray", anchor="mm")

    y = 120
    for i, (nickname, best_time) in enumerate(leaderboard, start=1):
        draw.text((50, y), f"{i}. {nickname}", font=font_text, fill="black")
        draw.text((width - 50, y), f"{best_time:.3f} сек", font=font_text, fill="black", anchor="rm")
        y += row_height
